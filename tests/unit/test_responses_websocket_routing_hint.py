...
@pytest.mark.parametrize("tier", ["priority", None])
async def test_subscription_handshake_uses_normalized_request_not_inbound_hint(
    ...
    # Then RED is an assertion at an existing transport seam, not a new kwarg.
    request = native.websocket.await_args.args[0]
    assert isinstance(request, NativeWebSocketRequest)
    expected = "model=gpt-5;tier=priority" if tier is not None else "model=gpt-5"
...
async def test_unopted_client_and_live_do_not_forward_hint(monkeypatch: pytest.MonkeyPatch, live: bool) -> None:
    ...
    with anyio.fail_after(5):
        if live:
            ...
            upstream = await client.connect_responses_websocket(
                headers, "test-api-key", None, allow_direct_egress=True
            )
        await upstream.close()
