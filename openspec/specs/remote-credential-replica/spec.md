# remote-credential-replica Specification

## Purpose

Define a dedicated routing replica that consumes credentials from a source
codex-lb dashboard without sharing its database or exchanging refresh tokens.
...
Successful synchronization MUST preserve source account identity and apply source pause, disable and removal state. Credential rotation MUST update the existing replica account rather than create duplicate accounts or override unrelated local request accounting. Source failure MUST NOT be interpreted as a successful empty snapshot.
...
- **AND** its local request history is retained

#### Scenario: All source accounts are removed
- **GIVEN** previously mirrored accounts
- **WHEN** a valid successful source snapshot contains an empty account list
- **THEN** all previously mirrored accounts become unavailable for new requests
