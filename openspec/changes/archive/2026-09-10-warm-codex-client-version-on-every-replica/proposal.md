...
- No new settings, schema, API or dashboard changes. Each enabled replica uses the existing one-hour success cache. Failed lookups can retry on later ticks; a cold request can still use the fallback before warm-up completes. Disabled schedulers do not fetch. Operator fallback overrides remain authoritative.
